...
class MovingActor(Actor):
    ...
    def update(self, dt):
        if self.planet is None:
            return
        dist = self._distance
        self.angle = (self.angle + self.rotation_speed * dt) % (math.pi * 2)
    ...
    def avoid_force(self):
        ...
        for obj in Planet.instances:
            ...
            if 0 < t < l:
                ...
                if dist < obj.cshape.r and (closest is None or dist < closest_dist):
    ...
    def process_player_collisions(self):
        ...
        for obj in self.coll_man.iter_colliding(player):
            if isinstance(obj, Pickup):
                self.add(ActorExplosion(obj.position, obj.particles))
                obj.kill()
            else:
                self.add(ActorExplosion(player.position, player.particles))
                player.kill()
...
if __name__ == '__main__':
    cocos.director.director.init(width=850, height=600, caption='Gravitation')
